...
public class MPcode{
    ...
    /** additional cache for holding the active trajectory point (both for left Talon and right Talon) */
    double _posLeftWheels=0,_velLeftWheels=0,_headingLeftWheels=0, _headingRightWheels=0, _posRightWheels=0, _velRightWheels=0;
     ...
     * motion profile.
     */
    private TalonSRX _talonLeft; //making the left Talon
    private Trajectory path; //the trajectory we are using (not sure if it will work)
    private TalonSRX _talonRight; //making the right Talon
    /**
    ...
    class PeriodicRunnable implements java.lang.Runnable {
        public void run() {  _talonLeft.processMotionProfileBuffer();  _talonRight.processMotionProfileBuffer();  } //I believe the processMotionProfilingBuffer should be done for both talons
    }

    Notifier _notifer = new Notifier(new PeriodicRunnable()); // No idea what this means
     ...
     */
    public MPcode(TalonSRX talon1, TalonSRX talon2, Trajectory trajectory) { // when we instanitiate this, we can set which 2 talons to use
        _talonLeft = talon1;
        _talonRight = talon2;
        path = trajectory; // not sure if this will work, since trajectory is an abstract parent class, while left and right trajectory are childrens
        /*
    ...
    public void reset() {
         ...
         * sitting in memory.
         */
        _talonLeft.clearMotionProfileTrajectories(); //Clearing the Trajectories in both left and right Talons
    ...
    public void control() {
        ...
        /* Get the motion profile status every loop */ //Each individual Talon should have its own heading pos, and velocity
        _talonLeft.getMotionProfileStatus(_status);
        _talonRight.getMotionProfileStatus(_status);
        _headingLeftWheels = _talonLeft.getActiveTrajectoryHeading();
        _posLeftWheels = _talonLeft.getActiveTrajectoryPosition();
        _velLeftWheels = _talonLeft.getActiveTrajectoryVelocity();
        _headingRightWheels = _talonRight.getActiveTrajectoryHeading();
        _posRightWheels = _talonRight.getActiveTrajectoryPosition();
        _velRightWheels = _talonRight.getActiveTrajectoryVelocity();
        /* printfs and/or logging */
        Instrumentation.process(_status, _posLeftWheels, _velLeftWheels, _headingLeftWheels);
        Instrumentation.process(_status, _posRightWheels, _velRightWheels, _headingRightWheels);
    ...
    private void startFilling(double[][] profile, int totalCnt, TalonSRX talon) { // Should work for all Talons
        ...
        for (int i = 0; i < totalCnt; ++i) {
            ...
            point.timeDur = GetTrajectoryDuration1((int)profile[i][2]);
            point.zeroPos = false;
            if (i == 0)
